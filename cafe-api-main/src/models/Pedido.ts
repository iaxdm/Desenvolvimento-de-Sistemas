...
export const PedidoModel = {
    ...
    async criar(itens: INovoItemPedido[]): Promise<{id: number}>{
        ...
        try {
            ...
            for (const item of itens){ //Verifica todos os itens da lista
    ...
    async deletarPedido (id:number): Promise<boolean> {
        ...
        try {
            await client.query("BEGIN");
            const queryItens = "SELECT produto_id, quantidade FROM itens_pedido where pedido_id = $1"
            const {rows} = await client.query(queryItens, [id]);
            for (const row of rows){
                await client.query("UPDATE produtos SET estoque = estoque + $1 WHERE id = $2",[row.quantidade, row.produto_id]);
            }
